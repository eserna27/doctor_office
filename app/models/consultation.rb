class Consultation < ApplicationRecord
  ...
  def self.doctor_day_consultations(doctor_id:, time:)
    where("doctor_id = :doctor_id AND time >= :start and created_at < :end and status != :status",
      doctor_id: doctor_id, start: time.beginning_of_day, end: time.end_of_day, status: "cancel")
  end
